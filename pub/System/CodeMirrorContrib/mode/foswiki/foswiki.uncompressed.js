...
(function(CodeMirror) {
  ...
  var italicRegExp     = /\_(?:\S+|(?:\S.*?\S))\_(?=$|[\s,.;:!?\)])/,
      ...
      externalLinkRegExp = /(?:file|ftp|gopher|https|http|irc|mailto|news|nntp|sip|telnet):(?:[^\s<>"]+[^\s*.,!?;:\)<|])/,

      abbrRegExp = new RegExp(
        "["+foswiki.RE.upper + ']{3,}s?\\b'
      ), 
  ...
  CodeMirror.defineMode("foswiki", function(config, parserConfig) {
    ...
    function tokenInline(stream, state) {
      var prevChar = stream.prev(), match, style, pos;
      ...
      if (typeof(prevChar) === "undefined" || prevChar.match(/\s|\(/)) { // simulate look behind
        ...
        if (!state.insideNoAutolink) {
          ...
          // acronym
/* DISABLED: this vastly overgenerates

          if (stream.match(abbrRegExp, 1)) {
            return "link";
          }
*/
        }
